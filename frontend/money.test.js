...
import { parseAmount, toCents, fromCents, splitIntoShares, formatMoney, MAX_CENTS, Decimal } from './money.js';
...
test('preserves cents throughout the supported range', () => {
  for (const value of ['90071992547409.91', '792281625142643375935439503.35', '0.01']) {
    ...
    assert.ok(shares.reduce((sum, share) => sum.plus(toCents(share.amount)), new Decimal(0)).eq(toCents(value)));
  }
  assert.equal(fromCents(MAX_CENTS.plus(MAX_CENTS)), '1584563250285286751870879006.70');
  assert.ok(toCents('1584563250285286751870879006.7').eq(MAX_CENTS.times(2)));
