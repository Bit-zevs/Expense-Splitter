...
Калькулятор накапливает целые копейки в BigInteger. Для итоговых балансов и переводов
проверяется точная представимость в decimal, а не лимит одного расхода;
перестановка расходов не влияет на результат.
